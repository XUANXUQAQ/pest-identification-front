  ...
  <div>
    <div style="margin: 10px">
      ...
      <span style="position: fixed; left: 50%">
        ...
        <span style="font-size: 20px"> 搜索目 </span>
  ...
  methods: {
    ...
    async getData(selectValue) {
      ...
      if (this.searchValue) {
        if (selectValue === 'id') {
          ...
          result = await this.$orderApi.selectOrderByName(
            this.cur,
            this.totalPage,
            this.searchValue,
          );
    ...
    confirmEdit() {
      ...
      if (this.isUpdate) {
        // 更新
        return this.$orderApi
          .updateOrder(this.form.id, this.form.code, this.form.name)
          .then(() => {
            this.getData();
          })
          .catch((res) => {
      ...
      // 添加
      return this.$orderApi
        .insertOrder(0, this.form.code, this.form.name)
        .then(() => {
          this.getData();
        })
        .catch((res) => {
