<template>
  <div>
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column align="center" label="代码" :width="(screenWidth / 6) + 'px'">
        <template slot-scope="{row}">
          <span>{{ row.code }}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" label="名称" :width="(screenWidth / 6) + 'px'">
        <template slot-scope="{row}">
          <span>{{ row.name }}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" label="科名称" :width="(screenWidth / 6) + 'px'">
        <template slot-scope="{row}">
          <span>{{ row.family_name }}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" label="目名称" :width="(screenWidth / 6) + 'px'">
        <template slot-scope="{row}">
          <span>{{ row.order_name }}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" label="Actions" :width="(screenWidth / 6) + 'px'">
        <template slot-scope="{row}">
          <el-button
            type="primary"
            size="small"
...
export default {
  ...
  methods: {
    ...
    async getData(selectValue) {
      ...
      if (this.searchValue) {
        if (selectValue === 'id') {
          result = await this.$genusApi.selectGenusById(this.searchValue);
        } else if (selectValue === 'name') {
          result = await this.$genusApi.selectGenusByName(this.cur, this.totalPage, this.searchValue);
        } else {
          result = await this.$genusApi.selectAllGenus(this.cur, this.totalPage);
        }
      } else {
        result = await this.$genusApi.selectAllGenus(this.cur, this.totalPage);
      }
    ...
    async getAllFamilies() {
      const resp = await this.$familyApi.selectAllFamilies(1, 100);
      this.allFamilies = resp.data;
    },
    showAddForm() {
    ...
    autoInsertOrder() {
      ...
      this.$orderFamilyApi.selectOrderByFamilyId(familyId).then((res) => {
        res.data.forEach((obj) => {
          this.form.orderId = obj.order_id;
          this.form.orderName = obj.order_name;
        });
      });
    },
  ...
  computed: {
    ...
    familyIdCompute() {
      return this.form.familyId;
    },
  },
  watch: {
    familyIdCompute() {
      this.autoInsertOrder();
    },
  },
};
</script>
