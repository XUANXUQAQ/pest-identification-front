  ...
  <div>
    <div style="margin: 10px">
      ...
      <span style="position: fixed; left: 50%">
        ...
        <span style="font-size: 20px"> 搜索属 </span>
    ...
    >
      ...
      <el-table-column align="center" label="科名称">
        <template slot-scope="{ row }">
          <span>{{ row.family_name }}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" label="目名称">
        <template slot-scope="{ row }">
...
export default {
  ...
  methods: {
    ...
    async getData(selectValue) {
      ...
      if (this.searchValue) {
        if (selectValue === 'id') {
          ...
          result = await this.$genusApi.selectGenusByName(
            this.cur,
            this.totalPage,
            this.searchValue,
          );
    ...
    confirmDelete() {
      ...
      return this.$genusApi
        .deleteGenus(this.form.id)
        .then(() => {
          this.getData();
        })
        .catch((res) => {
    ...
    confirmEdit() {
      ...
      if (this.isUpdate) {
        // 更新
        return this.$genusApi
          .updateGenus(this.form.id, this.form.code, this.form.name, this.form.familyId)
          .then(() => {
            this.getData();
          })
          .catch((res) => {
      ...
      // 添加
      return this.$genusApi
        .insertGenus(0, this.form.code, this.form.name, this.form.familyId)
        .then(() => {
          this.getData();
        })
        .catch((res) => {
